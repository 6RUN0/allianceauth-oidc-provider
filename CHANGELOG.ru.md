...
- Заметки к обновлению 0.6.0 назвали два заметных изменения DOT 3.4.1 и
  подали их как полный набор. Самыми болезненными они не были. DOT 3.4.1
  ещё и сравнивает redirect URI точно (RFC 9700 §2.1), отвергая всякий
  запрос, в `redirect_uri` которого есть query-параметр, path-параметр,
  учётные данные или фрагмент, отсутствующие у зарегистрированного URI:
  RP, передающий через эту query-строку меняющиеся от запроса к запросу
  данные, ломается на каждой авторизации, пока URI не зарегистрирован
  целиком или данные не перенесены в `state`. Вместе с этим упущены:
  `REFRESH_TOKEN_EXPIRE_SECONDS` теперь проверяется при предъявлении
  refresh-токена, а не только подчисткой `cleartokens`; намеренно
  отозванный refresh-токен больше не принимается внутри ненулевого
  `REFRESH_TOKEN_GRACE_PERIOD_SECONDS`; `/o/authorized_tokens/` требует
  `collectstatic`, чтобы рисоваться со стилями, - DOT убрал ссылку на
  Bootstrap с CDN; а Django admin заменил прямое удаление access- и
  refresh-токенов действием "Revoke selected". Полный перечень и ссылка на
  страницу обновления у апстрима - в `README.ru.md`.
- Там же сказано, что `manage.py migrate` не выполняет ни одного
  SQL-запроса. Это верно для `allianceauth_oidc.0022` и для
  `oauth2_provider.0021` из DOT, но тот же прогон применяет и
  `oauth2_provider.0022`, добавляющую индекс на
  `oauth2_provider_refreshtoken.token_family`. На большой таблице
  refresh-токенов индекс строится не мгновенно, а на MySQL/MariaDB всё это
  время удерживается блокировка метаданных таблицы, так что окно
  обновления, рассчитанное по "ни одного SQL-запроса", рассчитано неверно.
...
- Поддерживаемый диапазон `django-oauth-toolkit` сужен с `>=3.4,<3.5` до
  `>=3.4.1,<3.5`. Причина - изменение отзыва, описанное ниже: тесты
  закрепляют семантику 3.4.1 и на 3.4.0 краснеют, так что заявлять
  поддержку 3.4.0 больше нельзя. Установкам, закреплённым на DOT 3.4.0,
  нужно перейти на 3.4.1. Отдельно от этого 3.4.1 переподписывает поля,
  унаследованные от абстрактной модели приложения (`verbose_name`):
  `makemigrations --check` краснеет, хотя схема не меняется. Миграция
  `0022` (выполните `manage.py migrate`) фиксирует новые подписи полей и
  не порождает ни строчки SQL - `sqlmigrate allianceauth_oidc 0022`
  печатает пустые `BEGIN; COMMIT;`.
- Вместе с DOT 3.4.1 ужесточается семантика отзыва - оба изменения в
  безопасную сторону и оба теперь закреплены тестами. Отзыв access-токена
  на `/o/revoke_token/` заодно отзывает привязанный к нему refresh-токен, а
  не оставляет его сиротой, способным тут же выпустить новый access; RP,
  который отзывал access-токен, чтобы отключить одно устройство, и
  продолжал обновляться по тому же гранту, теперь обязан пройти
  авторизацию заново. Второе: отзыв ограничен клиентом-владельцем токена -
  другой конфиденциальный клиент со своими действительными учётными
  данными больше не отзовёт чужие токены. Сам адрес при этом по-прежнему
  отвечает `200` согласно RFC 7009 §2.2 - статус никогда не выдаёт, был ли
  такой токен.
...
- **Ломающее:** поддерживаемый диапазон `django-oauth-toolkit` сужен с
  `>=3.2,<4` до `>=3.4,<3.5`. DOT 3.4 добавляет колонки
  `registration_source` / `cimd_expires_at` и расширяет `client_id` до
  255 символов на абстрактной модели приложения; миграция `0021`
  (обязательная — выполните `manage.py migrate`) выравнивает схему, а
  ограничение `<3.5` не даёт следующему минорному релизу DOT снова
  незаметно разойтись со схемой. Замечание для MariaDB/MySQL:
  расширенный уникальный индекс `client_id` занимает 1020 байт в
  `utf8mb4` — больше лимита 767 байт устаревших форматов строк InnoDB
  `COMPACT`/`REDUNDANT` (`ERROR 1071`). Миграция `0021` решает это
  сама: условный предварительный шаг переводит таблицу приложений на
  `DYNAMIC` (формат по умолчанию с MariaDB 10.2 / MySQL 5.7) до
  перестройки индекса и ничего не делает, если формат уже подходит
  или бэкенд не из семейства MySQL.
- Форма приложения в админке показывает поля провенанса DOT 3.4
  (`registration_source`, `cimd_expires_at`) только для чтения, а в
  списке появился фильтр по `registration_source`: при разборе
  инцидента оператор должен отличать саморегистрированного клиента от
  заведённого вручную, но редактировать поле нельзя — CIMD-механика
  DOT ветвится по нему в путь с сетевым обновлением метаданных.
- Endpoint авторизации теперь явно отклоняет JWT-упакованные запросы
  авторизации вместо молчаливого игнорирования: на `request` отвечает
  `error=request_not_supported`, на `request_uri` —
  `error=request_uri_not_supported` (OIDC Core 1.0 §6.1 / §6.2).
  Молчаливое игнорирование создавало риск подмены параметров: клиент
  считает действующими параметры подписанного объекта, а сервер
  работает по обычной строке запроса. Редирект с ошибкой уходит
  только на redirect_uri, проверенный по зарегистрированному набору
  клиента; при незарегистрированном или отсутствующем redirect_uri
  запрос по-прежнему получает страницу ошибки 400.

### Добавлено

- Системная проверка `allianceauth_oidc.W007`: помечает приложения,
  сочетающие `algorithm="HS256"` с `hash_client_secret=True`. На
  DOT >= 3.4 такие записи роняют подпись id_token необработанной
  ошибкой 500 на `/o/token/` и не сохраняются через админку. HS256-
  приложения, созданные на прежних версиях DOT с хэшированным по
  умолчанию секретом, находятся ровно в этом состоянии — переведите их
  на RS256 или перевыпустите секрет с `hash_client_secret=False` и
  выполните `manage.py check` после обновления.
- Системная проверка `allianceauth_oidc.W008`: предупреждает, когда
  включён `OAUTH2_PROVIDER['CIMD_ENABLED']` или `['DCR_ENABLED']`.
  Саморегистрация клиентов создаёт приложения с пустым белым списком
  states/groups, который политика доступа трактует как «разрешено
  каждому пользователю с `access_oidc`», — это несовместимо с моделью
  белых списков, которую обеспечивает провайдер.
- Запросы авторизации с `acr_values` теперь дают в выпущенном
  id_token клейм `acr: "0"` (OIDC Core 1.0 §15.1). View сворачивает
  запрос в эквивалентный по спецификации необязательный член
  `claims.id_token.acr`, который переживает consent-цикл DOT и обмен
  кода; присланный клиентом параметр `claims` никогда не
  перезаписывается.
- Харнес конформанс-сьюта OpenID Foundation в `tests/conformance`:
  докеризованный сьют (запиненный релиз), REST-API-раннер планов с
  детерминированной автоматизацией headless-браузера, по-модульный
  экспорт результатов и леджер ожидаемых падений. Сертификационный
  план OIDC Core Basic проходит целиком (2 принятых предупреждения по
  покрытию клеймов, 4 ожидаемых пропуска на стороне сьюта).

### Исправлено

- Анонимные запросы с `prompt=none` теперь возвращают
  `error=login_required` согласно OIDC Core 1.0 §3.1.2.6. Сам по себе
  DOT 3.4 отдаёт для недоверенных клиентов `error=consent_required`.
  Перезапись работает по разобранной строке запроса, поэтому
  зарегистрированный `redirect_uri` со встроенным литералом параметра
  ошибки не может её сбить.
- Системная проверка `allianceauth_oidc.W006` и команда
  `oidc_fix_uuid_columns` больше не срабатывают на Django 4.2
  (стек AA 4.x). Раньше обе смотрели только на версию MariaDB, но
  переполнение native-uuid возникает, лишь когда 36-символьную форму
  пишет сам *Django* (`connection.features.has_native_uuid_field`,
  Django >= 5): под Django 4.2 колонка `char(32)` корректна, так что
  проверка предупреждала ложно, а команда преждевременно
  конвертировала колонки.
- POST-запросы к `/o/userinfo/` с access-токеном в теле формы больше
  не падают с 403: переопределение dispatch ради заголовков
  Cache-Control (OIDC Core 1.0 §5.3.2) незаметно потеряло
  `csrf_exempt` DOT, и CSRF-middleware Django отклонял
  аутентифицированные токеном POST-запросы.
