...
def _max_age_expired(request: HttpRequest, user: UserLike | None) -> bool:
    ...
    ``max_age=0`` is the spec's "always reauthenticate" sentinel:
    any non-zero elapsed time exceeds it. Malformed values
    (non-integer or negative) parse to ``None`` and short-circuit
    to False — see :func:`enforce_reauth` for the security
    rationale (never bouncing a user to login based on an
    unparseable, attacker-controlled value).
    """
    ...
    if last_login is None:
        ...
        # accounts); treating it as "infinitely old" matches the
        # spec's "elapsed time since last authentication" semantics
        # — the elapsed time is undefined, so any finite cap is
        # exceeded.
        return True
...
def _reauth_next_url(request: HttpRequest, *, strip_prompt_login: bool) -> str:
    """
    Compose the ``next`` URL used by the LOGIN_URL redirect.

    Mutates a copy of ``request.GET`` rather than the live
    QueryDict (Django guards live request data against accidental
    edits). When ``strip_prompt_login`` is True, the ``"login"``
    token is removed from the space-delimited ``prompt`` parameter;
    if it was the only value, the parameter is dropped entirely.
    The path is taken from ``request.path`` so the same builder
    works for both the GET and the (post-promotion) POST authorize
    endpoints.
    """
...
def enforce_reauth(
    ...
    * ``prompt=login`` — RP explicitly demands a fresh
      authentication event. The user is logged out and bounced to
      login with ``next`` pointing back to the authorize endpoint.
      ``prompt=login`` is stripped from the ``next`` URL so the
      post-login replay does not retrigger the gate; any other
      ``prompt`` values (e.g. ``prompt=consent``) survive the
      strip.
    * ``max_age=N`` — RP caps the acceptable age (seconds) of the
      End-User's last authentication. When
      ``now - user.last_login > N`` (or ``last_login`` is missing
      entirely), the gate behaves identically to ``prompt=login``
      except that ``max_age`` is preserved in the ``next`` URL:
      after re-auth ``last_login`` is fresh, so the next-iteration
      check passes naturally. Conformance: dropping ``max_age``
      from the replay would remove the spec-mandated requirement
      that ``auth_time`` be present in the id_token, so it MUST
      survive the round-trip.

    Malformed ``max_age`` (non-integer, negative) is ignored
    rather than treated as ``0`` — never bouncing a user to login
    based on an unparseable, attacker-controlled value.
    """
    prompt_raw = str(request.GET.get("prompt") or "")
    prompt_values = prompt_raw.split()
    prompt_login = "login" in prompt_values
    if not (prompt_login or _max_age_expired(request, user)):
        return None
    next_url = _reauth_next_url(request, strip_prompt_login=prompt_login)
    logout(request)
...
class AuthAuthorizationView(AuthorizationView):
    ...
    def _dispatch_inner(
        ...
        reauth_response = enforce_reauth(request, user)
