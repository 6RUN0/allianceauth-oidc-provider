...
DOT вместе с этим пакетом и выполните `python manage.py migrate`: миграция `0022` лишь
фиксирует новые подписи полей и не выполняет ни одного SQL-запроса, но пока она не применена,
`manage.py` будет о ней напоминать.

**Это меняет поведение, заметное RP**, так что прочитайте раздел до обновления нагруженной
установки. Оба изменения пришли из самого DOT, и оба закрывают дыру:

- **Отзыв access-токена теперь отзывает и привязанный к нему refresh-токен.** Раньше
  ...
  после обновления пользователи один раз пройдут авторизацию заново.
