...
> английский [README.md](https://github.com/6RUN0/allianceauth-oidc-provider/blob/current/README.md).
...
Исходник диаграммы — `assets/diagrams/policy-flow.d2`; перерисовать через `make diagrams` после правок.
...
1. Поставьте форк из PyPI. Форк опубликован под именем
   ...
   Для Prometheus-метрик поставьте extra:
   `pip install allianceauth-oidc-provider-eveo7[metrics]` (см. раздел
   [Наблюдаемость](#наблюдаемость--prometheus-метрики)).

   **Не ставьте** одновременно `allianceauth-oidc-provider` и
   `allianceauth-oidc-provider-eveo7` в одно окружение — оба
   распаковываются в директорию `allianceauth_oidc/`, pip откажет при
   второй установке с конфликтом файлов. Удалите апстримный пакет, если
   он установлен.
...
6. Проверьте, что провайдер отвечает:

   ```sh
   curl -s https://your.host/o/.well-known/openid-configuration | python -m json.tool
   ```

   В ответе должны быть `issuer`, `authorization_endpoint`, `jwks_uri` и
   `backchannel_logout_supported`.

> **Примечание.** Если у вас кастомный шаблон логина (`authentication/templates/public/login.html`),
...
### Нижняя граница `django-oauth-toolkit` поднята до 3.4.1

Поддерживаемый диапазон теперь `>=3.4.1,<3.5`, DOT 3.4.0 больше не поддерживается. Обновите
DOT вместе с этим пакетом и выполните `python manage.py migrate`.

Этот `migrate` применит три миграции, и схему меняет лишь одна из них.
`allianceauth_oidc.0022` фиксирует новые подписи полей и не выполняет ни одного SQL-запроса
(`sqlmigrate allianceauth_oidc 0022` печатает голый `BEGIN; COMMIT;`), то же и у собственной
`oauth2_provider.0021` из DOT. А `oauth2_provider.0022` добавляет индекс на
`oauth2_provider_refreshtoken.token_family`. Планируйте окно именно под неё: на большой
таблице refresh-токенов индекс строится не мгновенно, а на MySQL/MariaDB всё это время
удерживается блокировка метаданных таблицы.

**DOT 3.4.1 - релиз с исправлениями безопасности, и он меняет поведение, заметное и RP, и
пользователям.** Прочитайте раздел до обновления нагруженной установки. Полный перечень - на
странице
[Upgrading to 3.4.1](https://django-oauth-toolkit.readthedocs.io/en/3.4.1/upgrade.html#upgrading-to-3-4-1)
у апстрима; ниже - та часть, что доходит до установки с этим пакетом, от самого болезненного
к остальному.

- **Redirect URI сравниваются точно** (RFC 9700 §2.1, то же повторяет OIDC Core §3.1.2.1).
  Закрыты четыре способа разойтись с зарегистрированным URI и всё же совпасть с ним:
  незарегистрированный query-параметр, path-параметр (`/cb;evil=1`), учётные данные
  (`https://evil@example.com/cb`) и фрагмент. RP, который передаёт меняющиеся от запроса к
  запросу данные через query-строку `redirect_uri`, теперь получит `invalid_request` на каждой
  авторизации. Либо зарегистрируйте URI целиком вместе с query и в том же порядке параметров,
  либо перенесите эти данные в `state` - он для этого и существует. Приложения, у которых в
  `redirect_uris` нет query-части, изменение не затрагивает. Регистрация ужесточена
  соответственно: URI, оканчивающийся на `#`, теперь падает с `ValidationError` при сохранении,
  а не сохраняется, чтобы затем никогда ничего не авторизовать. В обратную сторону: схемы
  частного использования из RFC 8252 §7.1 (`com.example.app:/oauth2redirect`, один слеш)
  наконец регистрируются, и клиентам нативных приложений больше не нужен обходной путь.
- **Отзыв access-токена отзывает и привязанный к нему refresh-токен.** Раньше
  refresh-токен оставался сиротой и тут же выпускал новый access-токен взамен. RP, который
  отзывает access-токен, чтобы отключить одно устройство, и продолжает обновляться по тому же
  гранту, теперь будет отправлен на повторную авторизацию. Если так устроен выход из системы,
  после обновления пользователи один раз пройдут авторизацию заново. То же теперь верно для
  кнопки отзыва на `/o/authorized_tokens/`, которая раньше удаляла только access-токен.
- **Отзыв ограничен клиентом-владельцем токена.** Второй конфиденциальный клиент со своими
  действительными учётными данными больше не отзовёт чужие токены. По RFC 7009 §2.2 адрес
  по-прежнему отвечает `200` - статус никогда не выдаёт, существовал ли токен, - поэтому RP,
  который полагался на отзыв чужих токенов, увидит успех без всякого эффекта, а не ошибку.
  Проверьте это до обновления, если на одной установке Auth работает несколько клиентов.
- **`REFRESH_TOKEN_EXPIRE_SECONDS` проверяется при предъявлении refresh-токена**, а не только
  подчисткой `cleartokens`. Если настройка задана, а задача чистки так и не была поставлена в
  расписание ([Периодическая чистка](#периодическая-чистка-истёкших-токенов-celery-beat)),
  refresh-токены старше этого возраста продолжали работать; теперь token-адрес их отвергает.
  Установок, где настройка не задана, изменение не касается.
- **Намеренно отозванный refresh-токен больше не принимается внутри
  `REFRESH_TOKEN_GRACE_PERIOD_SECONDS`.** Льготный период существует, чтобы прикрыть токен,
  который клиент повторяет, не получив ответа с ротацией; он же прикрывал и токены, убитые
  через `/o/revoke_token/`, выход из системы или admin. Уязвимы были только установки, поднявшие
  окно выше значения по умолчанию `0`, и только они увидят изменение.
- **`/o/authorized_tokens/` рисуется без стилей, пока не выполнен `collectstatic`.** Шаблоны DOT
  перестали тянуть Bootstrap со стороннего CDN и теперь подключают таблицу стилей из самого
  пакета, через `staticfiles`. Страницы согласия, подтверждения выхода и отказа в доступе,
  которые поставляет этот пакет, наследуются от базового шаблона Alliance Auth, и их изменение
  не затрагивает.
- **В Django admin access- и refresh-токены больше нельзя удалить**, вместо этого есть действие
  "Revoke selected". Прямое удаление access-токена оставляло его refresh-токен сиротой,
  способным и дальше выпускать токены, так что действие теперь единственный верный путь. У
  `Grant` и `IDToken` обычное удаление осталось.
...
cross-module namespacing `aa_<module>_*` — лежит в [docs/METRICS.ru.md](https://github.com/6RUN0/allianceauth-oidc-provider/blob/current/docs/METRICS.ru.md).
...
Контракт retry / dead-letter — в [docs/BACK_CHANNEL_LOGOUT.ru.md](https://github.com/6RUN0/allianceauth-oidc-provider/blob/current/docs/BACK_CHANNEL_LOGOUT.ru.md).
...
| `oidc_fix_uuid_columns` | Привести колонки `UUIDField` из DOT (`oauth2_provider_idtoken.jti`, `oauth2_provider_refreshtoken.token_family`) к нативному типу `uuid`, который Django ожидает на MariaDB `>= 10.7` (чинит ошибку `1406 Data too long` на `/o/token/` для `jti` и при ротации refresh-токенов для `token_family`, после апгрейда MariaDB через границу 10.7). Сохраняет nullability каждой колонки; no-op на всех остальных бэкендах / уже сконвертированных колонках. См. [docs/MARIADB.ru.md](https://github.com/6RUN0/allianceauth-oidc-provider/blob/current/docs/MARIADB.ru.md). | да | `--dry-run`, `--format` |
...
> **Важно.** Нужны обе настройки. `ACCESS_TOKEN_GENERATOR` принимает dotted-path строку,
...
[docs/JWT_ACCESS_TOKENS.ru.md](https://github.com/6RUN0/allianceauth-oidc-provider/blob/current/docs/JWT_ACCESS_TOKENS.ru.md). Рецепт в §7
...
Sub-only [back-channel logout](https://github.com/6RUN0/allianceauth-oidc-provider/blob/current/docs/BACK_CHANNEL_LOGOUT.ru.md)
...
| `allianceauth_oidc.E006` | Error | Опасная комбинация с конкретной жертвой: `ALLIANCEAUTH_OIDC_LOGOUT_URI_ALLOW_PRIVATE=True` И `DEBUG=False` И хотя бы у одного `AllianceAuthApplication` непустой `backchannel_logout_uri`. Подписанные `logout_token` JWT уходили бы на private-IP-таргеты в production-shaped окружении. | Установите `ALLIANCEAUTH_OIDC_LOGOUT_URI_ALLOW_PRIVATE=False` в проде. Чтобы явно принять trade-off на изолированном лабе / air-gapped staging'е, дополнительно установите `ALLIANCEAUTH_OIDC_ALLOW_PRIVATE_BCL_IN_PRODUCTION=True` — оба переключателя должны быть выставлены, чтобы опасный путь работал без ошибки. |
...
| `allianceauth_oidc.W006` | Warning | Колонка `UUIDField` из DOT (`oauth2_provider_idtoken.jti` и/или `oauth2_provider_refreshtoken.token_family`) всё ещё `char(32)` на бэкенде MariaDB `>= 10.7`. Django 5.x считает такую MariaDB носителем нативного типа `uuid` и пишет значение в 36-символьной форме с дефисами, что переполняет legacy-колонку с `1406 Data too long` — валит выдачу id_token на `/o/token/` (`jti`) либо ротацию refresh-токенов (`token_family`). Помечена как database (выполняется на `migrate` / `check --database`). | Запустите `manage.py oidc_fix_uuid_columns`, чтобы сконвертировать колонки к нативному типу `uuid`, который Django теперь ожидает. См. [docs/MARIADB.ru.md](https://github.com/6RUN0/allianceauth-oidc-provider/blob/current/docs/MARIADB.ru.md). |
...
| Issuer | то, что отдаёт поле `issuer` из `https://auth.example.com/o/.well-known/openid-configuration` — каноническая форма без хвостового слэша (`https://your.host/o`); копируйте дословно, строгие валидаторы рубят запрос на любом расхождении (см. [Endpoint'ы](#endpointы)) |
...
- Только authorization-code flow; public-клиенты вне scope'а.
- Session-scoped (`sid`) back-channel logout отложен до feature v2 (сегодня только sub).
- Встроенного rate-limiting на `/o/token/` и `/o/authorize/` нет — это
  ответственность оператора (см.
  [усиление безопасности на стороне деплоя](#усиление-безопасности--на-стороне-деплоя)).
- Свой `/metrics` не монтируется (делегировано django-prometheus).
...
| `tests_mariadb` | прогон тестов на реальной MariaDB вместо sqlite (см. [docs/MARIADB.ru.md](https://github.com/6RUN0/allianceauth-oidc-provider/blob/current/docs/MARIADB.ru.md)) | нет |
...
[`_nox/makefile.py`](https://github.com/6RUN0/allianceauth-oidc-provider/blob/current/_nox/makefile.py); поменяйте таргет там (или добавьте новый под сессию) и
...
триажа — лежат в [tests/conformance/README.md](https://github.com/6RUN0/allianceauth-oidc-provider/blob/current/tests/conformance/README.md).

## Ссылки

- Changelog: <https://github.com/6RUN0/allianceauth-oidc-provider/blob/current/CHANGELOG.md>
- Баг-трекер: <https://github.com/6RUN0/allianceauth-oidc-provider/issues>
- Upstream-проект:
  <https://github.com/Solar-Helix-Independent-Transport/allianceauth-oidc-provider>

## Лицензия

MIT — см. [LICENSE](https://github.com/6RUN0/allianceauth-oidc-provider/blob/current/LICENSE).
