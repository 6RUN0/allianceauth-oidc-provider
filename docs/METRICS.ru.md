...
| `kid`        | JWK thumbprint                                                        | Идентификатор активного signing-ключа. Cardinality ограничен политикой ротации (обычно 1-3 активных). **Зарезервированное** имя лейбла — сейчас ни одна метрика из inventory его не эмиттит; задокументирован здесь, чтобы возможная будущая JWKS-rotation метрика переиспользовала каноническое имя, а не выдумала синоним. |
...
| `aa_oidc_code_reuse_audit_misses_total`| Counter   | `client_id`               | `auth_provider.py`    | `_handle_potential_code_reuse` инкрементирует когда для предъявленного `code` нет строки в `IssuedCodeAudit`. Atomic-wrap `save_bearer_token` + `_record_code_issuance` закрывает race-window-источник; счётчик теперь срабатывает исключительно на never-issued кодах (fuzzers / replay на чужой провайдер). Коррелировать с сигналом `oidc_code_reuse_detected` — метрики должны быть непересекающимися. |
| `aa_oidc_code_audit_skipped_total`     | Counter   | `reason`                  | `_metrics.py`         | `_record_code_issuance` инкрементирует когда вставка строки пропущена по defensive-причине (сегодня единственная — `no_client`: oauthlib дошёл до `save_bearer_token` без резолвимого клиента). Ненулевой rate деградирует overlay RFC 6749 §10.5 SHOULD для затронутых обменов; investigate валидатор-стек. Значения reason берутся из `constants.CODE_AUDIT_SKIPPED_REASONS`. |
