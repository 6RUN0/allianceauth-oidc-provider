...
PII-at-rest сжимается пропорционально. Та же логика — в WikiJS-рецепте
из README; в JWT-режиме короткий TTL становится default'ом, а не
частным случаем.
...
1. Запустите `manage.py migrate`, чтобы в схеме появилась колонка
   `access_token_format`. Чистая установка применяет все миграции
   до последней; сама колонка добавлена в
