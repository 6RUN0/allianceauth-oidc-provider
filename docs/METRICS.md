...
| `kid`        | A JWK thumbprint                                                      | Active signing key identifier. Cardinality bounded by rotation policy (typically 1-3 active). **Reserved** label name — not currently emitted by any metric in the inventory; documented here so a future JWKS-rotation metric reuses the canonical name instead of inventing a synonym. |
...
| `aa_oidc_code_reuse_audit_misses_total`| Counter   | `client_id`             | `auth_provider.py`    | `_handle_potential_code_reuse` increments when no `IssuedCodeAudit` row exists for the presented code. The atomic wrap of `save_bearer_token` + `_record_code_issuance` closes the race-window source; the counter now exclusively fires on never-issued codes (fuzzers / wrong-provider replays). Correlate against the `oidc_code_reuse_detected` signal — the two metrics should be disjoint. |
| `aa_oidc_code_audit_skipped_total`     | Counter   | `reason`                | `_metrics.py`         | `_record_code_issuance` increments when the row insert is skipped for a defensive reason (today: `no_client` — oauthlib reached `save_bearer_token` without a resolvable client). Non-zero rate degrades the RFC 6749 §10.5 SHOULD overlay for the affected exchanges; investigate the upstream validator stack. Reason values are drawn from `constants.CODE_AUDIT_SKIPPED_REASONS`. |
| `aa_oidc_audit_receiver_failures_total`| Counter   | `signal`, `receiver_dispatch_uid` | `_metrics.py`         | `signals.dispatch_audit_signal` increments per receiver that raised during `send_robust`. Single counter covers all four audit signals (`oidc_token_issued`, `oidc_code_reuse_detected`, `oidc_token_introspected`, `oidc_logout_dispatched`). Non-zero rate means the audit pipeline is silently dropping events for at least one downstream consumer (SIEM forwarder, custom hook). The `receiver_dispatch_uid` label recovers the Django `dispatch_uid` the receiver registered under (project-internal constants in `constants.py`); falls back to receiver `__qualname__` when the receiver was connected without an explicit uid. |
